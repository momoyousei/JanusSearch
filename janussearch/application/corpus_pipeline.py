...
PRESENTATION_LEVELS = ("poster", "oral", "bestpaper", "unknown", "not_applicable")
NON_PRESENTATION_VENUES = {"TPAMI", "VLDB"}
...
def normalize_presentation_level(value: Any, venue: Any = None) -> str:
    """Normalize presentation semantics without inventing a poster decision."""
    if ensure_str(venue).upper() in NON_PRESENTATION_VENUES:
...
def transform_record(raw: Dict[str, Any], context: FileContext) -> Dict[str, Any]:
    ...
    record["presentation_level"] = normalize_presentation_level(
        record.get("presentation_level"), context.venue
    )
...
def count_records_by(records: Sequence[Dict[str, Any]], key: str) -> Dict[str, int]:
    ...
    for record in records:
        if key == "track":
            ...
            normalized = normalize_presentation_level(
                record.get("presentation_level"), record.get("venue")
            )
        else:
